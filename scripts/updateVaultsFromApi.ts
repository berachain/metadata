/**
 * Script to query Berachain GraphQL API and update mainnet.json with missing vault entries
 * This script finds vaults without metadata and adds placeholder entries without modifying existing data
 */
...
const GRAPHQL_QUERY = `query GetVaults($where: GqlRewardVaultFilter, $pageSize: Int, $skip: Int, $orderBy: GqlRewardVaultOrderBy = bgtCapturePercentage, $orderDirection: GqlRewardVaultOrderDirection = desc, $search: String) {
  polGetRewardVaults(
    where: $where
    first: $pageSize
    skip: $skip
...
async function main() {
  ...
  try {
    ...
    const firstResponse = await queryVaults(apiUrl, 0, pageSize);
    ...
    while (skip < totalCount) {
      ...
      const response = await queryVaults(apiUrl, skip, pageSize);
